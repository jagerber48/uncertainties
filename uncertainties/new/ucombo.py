...
class UAtom:
    ...
    def __repr__(self):
        return f"{self.__class__.__name__} with UUID: {self.uuid}"
...
class UCombo:
    ...
    def __eq__(self, other: UCombo):
        self_expanded_dict = self.expanded_dict
        other_expanded_dict = other.expanded_dict
        for key, value in self_expanded_dict.items():
            if key not in other_expanded_dict:
                if value != 0:
                    return False
            other_value = other_expanded_dict[key]
            if other_value != value:
                if not isnan(value) and isnan(other_value):
                    return False
        return True
